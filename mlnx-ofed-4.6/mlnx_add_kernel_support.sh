...
if [ "X$distro" == "X" -a $deb -eq 0 ]; then
	...
	case "$distro_rpm" in
		...
		distro=xenserver6.5
		;;
		xenserver-release-7.2.0*)
		distro=xenserver7.2
		;;
		xenserver-release-7.1.0*)
		distro=xenserver7.1
		;;
		...
		distro=rhel6.9
		;;
		redhat-release*-6.10*|centos-release-6-10*|sl-release-6.10*)
		distro=rhel6.10
		;;
		...
		distro=rhel7.3
		;;
		redhat-release*-7.4*el7a*|centos-release-7-4*el7.centos.a*|sl-release-7.4*el7a*)
		distro=rhel7.4alternate
		;;
		redhat-release*-7.5*el7a*|centos-release-7-5*el7.centos.a*|sl-release-7.5*el7a*)
		distro=rhel7.5alternate
		;;
		redhat-release*-7.4*|centos-release-7-4*|sl-release-7.4*)
		distro=rhel7.4
		;;
		redhat-release*-7.5*|centos-release-7-5*|sl-release-7.5*)
		distro=rhel7.5
		;;
		redhat-release*-7.6*el7a*|centos-release-7-6*el7.centos.a*|sl-release-7.6*el7a*)
		distro=rhel7.6alternate
		;;
		redhat-release*-7.6*|centos-release-7-6*|sl-release-7.6*)
		distro=rhel7.6
		;;
		redhat-release-[4-8].[0-9]-*)
		# redhat-release-8.0-0.39 => rhel8.0
		distro=$(echo $distro_rpm | sed -r -e 's/redhat-release-([0-9])\.([0-9]+).*/rhel\1.\2/')
		;;
		...
		distro=ol6.8
		;;
		oraclelinux-release-6Server-9*)
		distro=ol6.9
		;;
		oraclelinux-release-6Server-10*)
		distro=ol6.10
		;;
		oraclelinux-release-7.[0-6]-*)
		distro=$(echo $distro_rpm | sed -r -e 's/oraclelinux-release-([0-9])\.([0-9]+).*/ol\1.\2/')
		;;
		sles-release-10-15.*)
		distro=sles10sp2
		;;
		...
		distro=sles11sp4
		;;
		sles-release-12\.4*|*SLES*release-12\.4*)
		distro=sles12sp4
		;;
		...
		;;
		sles-release-15.[1-9]*)
		distro=`echo ${distro_rpm} | sed -r -e 's/sles-release-(1[0125])\.([1-9]).*/sles\1sp\2/'`
		;;
		sles-release-15*|*SLES*release-15*)
		distro=sles15sp0
		;;
		fedora-release-1[4-9] | fedora-core-2[0-9])
		# fedora-release-25: distro=fc25
		distro=${distro_rpm/fedora-release-/fc}
		;;
		...
		;;
		euleros-release-2.0SP[235]*)
		sp=`echo ${distro_rpm} | sed -e 's/euleros-release-2\.0SP\(.\).*/\1/'`
...
ex ${ofed}/install.pl --tmpdir $tmpdir_logs --kernel-only --kernel $kernel --kernel-sources $kernel_sources --builddir $tmpdir $KMP $force $disabled_pkgs --build-only $distro1 $KMP_BUMP_VER $MLNX_EXTRA_FLAGS
...
# Rebuild --dpdk --upstream-libs repo dir
UPSTREAM_LIBS_REPO="${BINDIR}_UPSTREAM_LIBS"
if [ -e "$tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO" ]; then
	/bin/rm -rf $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO
	mkdir -p $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO
	cd $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO
	# all rdma-core based binaries
	rpms=`find ../${BINDIR}/UPSTREAM_LIBS/*`
	for p in $rpms
	do
		ex /bin/ln -s $p .
	done
	# kernel binaries
	rpms=`find ../${BINDIR}/ -maxdepth 1 -name "*ofed-scripts*" -o -name "*mlnx-fw-updater*" \
		-o -name "*mlnx-ofa_kernel-modules*" -o -name "*kernel*" -o -name "*knem*" -o -name "*srp*" \
		-o -name "*iser*" -o -name "*mlnx-ofed-kernel-modules*" -o -name "*mlnx-sdp*" -o -name "*mlnx-rds*" \
		-o -name "*mlnx-nfsrdma*" -o -name "*mlnx-nvme*" -o -name "*mlnx-rdma-rxe*"`
	for p in $rpms
	do
		case "$p" in
			*srptools* | *mlnx-ofed-kernel-only* | *supported_kernels*)
			continue
			;;
		esac
		ex /bin/ln -s $p .
...
if [ $skip_repo -eq 0 ]; then
	...
	# exclude UPSTREAM_LIBS from the repository
	if [ -e "$tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS" ]; then
		ex "/bin/mv -f $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS $tmpdir/$iso_dir/UPSTREAM_LIBS"
	fi
	# build info files
	DEBFLAG=
	DKMSFLAG=
	...
	if [ -e "$tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO" ]; then
		group_list="$group_list dpdk-upstream-libs"
	fi
	for group in $group_list
	do
		if [ "X${group}" == "Xdpdk-upstream-libs" ]; then
			# this one is special, it uses different --rpms-tdir fodler
			ex "/bin/mv -f $tmpdir/$iso_dir/UPSTREAM_LIBS $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS"
			$tmpdir/$iso_dir/$INSTALLER --dpdk --upstream-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check 2>/dev/null > $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO/${group}_packages.txt
			# user space only packages list
			$tmpdir/$iso_dir/$INSTALLER --dpdk --upstream-libs -p $DKMSFLAG -k $kernel --tmpdir $tmpdir_logs $disabled_pkgs $distro1 --skip-distro-check --user-space-only 2>/dev/null > $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO/${group}-user-only_packages.txt
			ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $tmpdir_logs --mofed $tmpdir/$iso_dir --rpms-path $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO --rpms-tdir $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM
			ex "/bin/mv -f $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS $tmpdir/$iso_dir/UPSTREAM_LIBS"
			rm -rf $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO/*txt >/dev/null 2>&1
		else
	...
	ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $tmpdir_logs --mofed $tmpdir/$iso_dir --rpms-tdir $tmpdir/$iso_dir/$BINDIR --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM $METAGROUPS
	...
	# update --dpdk --upstream-libs repo dir
	if [ -e "$tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO" ]; then
		/bin/cp -f $tmpdir/$iso_dir/${BINDIR}/*mlnx-ofed-kernel-only* $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO/ || true
	fi
	# create repo
	cd $tmpdir/$iso_dir
	...
	if [ $deb -eq 0 ]; then
		...
		if [ -e "$tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO" ]; then
			ex "/bin/mv -f $tmpdir/$iso_dir/UPSTREAM_LIBS $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS"
			ex createrepo -q $GROUPFILE $tmpdir/$iso_dir/$UPSTREAM_LIBS_REPO
			ex "/bin/mv -f $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS $tmpdir/$iso_dir/UPSTREAM_LIBS"
		fi
	...
	if [ -e "$tmpdir/$iso_dir/UPSTREAM_LIBS" ]; then
		ex "/bin/mv -f $tmpdir/$iso_dir/UPSTREAM_LIBS $tmpdir/$iso_dir/${BINDIR}/UPSTREAM_LIBS"
	fi
	cd $tmpdir
fi
