...
function run_program_dbg {    
    sudo stdbuf -o0 sh -c "gdb --ex \"handle SIGUSR2 nostop noprint\" --args $1 $AIFM_PATH/configs/client.config \
                           $MEM_SERVER_DPDK_IP:$MEM_SERVER_PORT"
}

function run_program_noht_dbg {
    sudo stdbuf -o0 sh -c "gdb --ex \"handle SIGUSR2 nostop noprint\" --args $1 $AIFM_PATH/configs/client_noht.config \
                           $MEM_SERVER_DPDK_IP:$MEM_SERVER_PORT"
}