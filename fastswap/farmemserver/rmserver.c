...
const size_t BUFFER_SIZE = 1024 * 1024 * 1024 * 128l;
...
static device *get_device(struct queue *q)
{
  ...
  if (!q->ctrl->dev) {
    ...
    ctrl->buffer = mmap(NULL, BUFFER_SIZE, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB|MAP_HUGE_SIZE, -1, 0);
