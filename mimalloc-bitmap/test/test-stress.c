...
static size_t di_used[DI_MAX_THREAD * DI_USED_SIZE];
static size_t di_total_size[DI_MAX_THREAD];
static size_t di_used_sum[DI_USED_SIZE];
...
static void stress(intptr_t tid) {
  ...
  size_t total_page = mi_thread_stats_fragmentation(DI_PAGE_SIZE, DI_UNIT_SIZE, &di_used[tid * DI_USED_SIZE]);
  di_total_size[tid] = total_page;
}
...
static void test_stress(void) {
  ...
  for (int n = 0; n < ITER; n++) {
    ...
    memset(di_used_sum, 0, DI_USED_SIZE * sizeof(size_t));
    for (size_t i = 0; i < THREADS; ++i){
      for (size_t j =0 ; j < DI_USED_SIZE ; ++j){
        di_used_sum[j] += di_used[(i * DI_USED_SIZE) + j];
      }
    }
    printf("\n\niter: %d\n", n);
    for (size_t j =0 ; j < DI_USED_SIZE ; ++j){
