...
#if 0
size_t mi_thread_stats_fragmentation(size_t page_size, size_t unit_size, size_t* used) mi_attr_noexcept {
  mi_heap_t* heap = mi_heap_get_default();

  // size_t page_size = 4096;
  // size_t unit_size = 64;
  // size_t used[page_size/unit_size];
  size_t used_buffer[MI_SMALL_PAGE_SIZE / page_size];

  size_t total_pages = 0;

  memset(used, 0, sizeof(size_t) * (page_size/unit_size));

  
  for (size_t i = 0; i < MI_BIN_HUGE; ++i){
    if(!is_bm_block_size(heap->pages[i].block_size)){
      continue;
    }
    mi_page_t* page = heap->pages[i].first;
    if (page == NULL)
      continue;
    do {
      int ret = _mi_page_fragmented(page, page_size, unit_size, used_buffer);
      mi_assert_internal(ret == 0);
      for (size_t j = 0; j < MI_SMALL_PAGE_SIZE / page_size; ++j) {
        mi_assert_internal(used_buffer[j] <= page_size/unit_size);
        used[used_buffer[j]] += 1;
        ++total_pages;
      }
      page = page->next;
    } while (page != NULL);
  ...
  mi_assert_internal(heap->next == NULL);
  return total_pages;
}


size_t mi_thread_stats_fragmentation_op(size_t page_size, size_t unit_size, size_t* used) mi_attr_noexcept {
  mi_heap_t* heap = mi_heap_get_default();

  // size_t page_size = 4096;
  // size_t unit_size = 64;
  // size_t used[page_size/unit_size];
  size_t used_buffer[64];
  ...
  for (size_t i = 0; i < MI_BIN_HUGE; ++i){
    ...
    if (page == NULL)
      continue;
    do {
      int ret = _mi_page_fragmented_op(page, page_size, unit_size, used_buffer);
      mi_assert_internal(ret == 0);
      for (size_t j = 0; j < 64; ++j) {
        mi_assert_internal(used_buffer[j] <= 64);
        used[j] += used_buffer[j];
        total_pages += used_buffer[j];
      }
      page = page->next;
    } while (page != NULL);
  }
