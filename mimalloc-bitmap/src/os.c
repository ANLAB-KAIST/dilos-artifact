...
static void* mi_unix_mmapx(void* addr, size_t size, size_t try_alignment, int protect_flags, int flags, int fd) {
  ...
  #endif
#if !defined(MAP_FIX_FIRST_TWO)
#define MAP_FIX_FIRST_TWO  0
#endif
  if (p==NULL) {
#ifndef DISABLE_SG
    p = size == 0x10000000 ? mmap(addr,size,protect_flags,flags|MAP_FIX_FIRST_TWO,fd,0) : mmap(addr,size,protect_flags,flags,fd,0);
#else
...
static void* mi_unix_mmap(void* addr, size_t size, size_t try_alignment, int protect_flags, bool large_only, bool allow_large, bool* is_large) {
  ...
  int flags = MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE | MAP_DDC;
  int fd = -1;
