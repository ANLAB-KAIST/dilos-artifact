...
static mi_block_t* _mi_bm_idx_to_block(const mi_bm_page_t* page, mi_bitmap_index_t bitmap_idx){
  const mi_page_t* inner = BM_TO_CONST_PAGE(page);
  mi_assert_internal(inner != NULL);
  mi_assert_internal(bitmap_idx < MI_SMALL_PAGE_SIZE / 64);
  uint8_t* const page_start = ((uint8_t*) _mi_page_segment(inner)) + (inner->segment_idx * MI_SMALL_PAGE_SIZE);
  return (mi_block_t*)((uint8_t*)page_start + (bitmap_idx * 64));
}

static mi_bitmap_index_t _mi_bm_block_to_idx(const mi_bm_page_t* page, mi_block_t* block){
  const mi_page_t* inner = BM_TO_CONST_PAGE(page);
  mi_assert_internal(inner != NULL);
  uint8_t* const page_start = ((uint8_t*) _mi_page_segment(inner)) + (inner->segment_idx * MI_SMALL_PAGE_SIZE);
  mi_bitmap_index_t idx = ((uint8_t*)block - page_start) / 64;
  mi_assert_internal(idx < MI_SMALL_PAGE_SIZE / 64);
  return idx;
}


static inline void __mi_free_bm_block_local(mi_bm_page_t* page, mi_block_t* block) {

      mi_page_t* inner = BM_TO_PAGE(page);
      mi_bitmap_index_t idx = _mi_bm_block_to_idx(page, block);
#ifdef MI_DEBUG
      const size_t num_field = 16;
#else
      const size_t num_field = 0;
#endif
      _mi_bitmap_unclaim_across_count_relaxed(inner->bm_free, num_field, idx, &inner->bm_super, MI_BITMAP_NUM_BITS(inner->xblock_size));
      inner->is_zero = false;
}

void mi_free_bm_block_local(mi_bm_page_t* page, mi_block_t* block) {
  __mi_free_bm_block_local(page, block);
} 

// Fast allocation in a page: just pop from the bitmap list.
// Fall back to generic allocation only if the bitmap is full.
extern inline void* _mi_bm_page_malloc(mi_heap_t* heap, mi_bm_page_t* page, size_t size) mi_attr_noexcept {
  mi_page_t* inner = BM_TO_PAGE(page);
  mi_assert_internal(inner->xblock_size==0||is_bm_block_size(inner->xblock_size));
#ifdef MI_DEBUG
  const size_t num_field = 16;
#else
  const size_t num_field = 0;
  ...
  if (mi_unlikely(inner->xblock_size==0 || !_mi_bitmap_try_find_from_claim_across_count_relaxed(inner->bm_free, num_field, &inner->bm_super, MI_BITMAP_NUM_BITS(inner->xblock_size), &bitmap_idx))) {
    return _mi_malloc_generic(heap, size); 
  }

  mi_block_t* block = _mi_bm_idx_to_block(page, bitmap_idx);

  mi_assert_internal(block != NULL && _mi_ptr_page(block) == inner);
  // pop from the free list
  inner->used++;

#if (MI_DEBUG>0)
  if (!inner->is_zero) { memset(block, MI_DEBUG_UNINIT, size); }
#elif (MI_SECURE!=0)
  block->next = 0;  // don't leak internal data
#endif

#if (MI_STAT>0)
  const size_t bsize = mi_page_usable_block_size(inner);
  ...
  mi_padding_t* const padding = (mi_padding_t*)((uint8_t*)block + mi_page_usable_block_size(inner));
  ptrdiff_t delta = ((uint8_t*)padding - (uint8_t*)block - (size - MI_PADDING_SIZE));
  mi_assert_internal(delta >= 0 && mi_page_usable_block_size(inner) >= (size - MI_PADDING_SIZE + delta));
  padding->canary = (uint32_t)(mi_ptr_encode(inner,block,inner->keys));
...
extern inline mi_decl_restrict void* mi_heap_malloc_small(mi_heap_t* heap, size_t size) mi_attr_noexcept {
  ...
  void* p = page->xblock_size >= BM_MIN_SIZE ? 
    _mi_bm_page_malloc(heap, PAGE_TO_BM(page), size + MI_PADDING_SIZE):
    _mi_page_malloc(heap, page, size + MI_PADDING_SIZE);
...
static inline void _mi_free_block(mi_page_t* page, bool local, mi_block_t* block)
{
  ...
  if (mi_likely(local)) {
    ...
    if(is_bm_page(page)){
      __mi_free_bm_block_local(PAGE_TO_BM(page), block);
    } else {
