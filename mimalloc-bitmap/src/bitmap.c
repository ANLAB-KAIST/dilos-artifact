...
// Try to atomically claim a sequence of 1 bit in a single
// field at `idx` in `bitmap`. Returns `true` on success.
bool _mi_bitmap_try_find_from_claim_across_relaxed(mi_bitmap_t bitmap, const size_t bitmap_fields,  mi_bitmap_super_t* super, mi_bitmap_index_t* bitmap_idx){
  UNUSED_RELEASE(bitmap_fields);

  size_t idx = mi_ctz(~*super);    // quickly find the first zero bit if possible
  if(mi_unlikely(idx == MI_INTPTR_BITS)){
    return false;
  }
  mi_assert_internal(idx < bitmap_fields);
  _Atomic(uintptr_t)*field = &bitmap[idx];
  uintptr_t prev = mi_atomic_load_relaxed(field);
  mi_assert_internal(prev != MI_BITMAP_FIELD_FULL);
  size_t bitidx = mi_ctz(~prev);  
  mi_assert_internal(bitidx != MI_INTPTR_BITS);
  uintptr_t next = prev | ((uintptr_t)1 << bitidx);
  mi_atomic_store_relaxed(field, next);

  if(mi_unlikely(next == MI_BITMAP_FIELD_FULL)){
    uintptr_t next_idx = *super | ((uintptr_t)1 << idx);
    *super = next_idx;
  }
  *bitmap_idx = mi_bitmap_index_create(idx, bitidx);
  return true;
...
bool _mi_bitmap_unclaim_across_relaxed(mi_bitmap_t bitmap, const size_t bitmap_fields, mi_bitmap_index_t bitmap_idx, mi_bitmap_super_t* super) {
  UNUSED_RELEASE(bitmap_fields);
  ...
  _Atomic(uintptr_t)*field = &bitmap[idx];
  uintptr_t prev = mi_atomic_load_relaxed(field);
  mi_assert_internal(prev != 0);
  mi_assert_internal((prev & ((uintptr_t)1 << bitidx)) != 0);
  mi_atomic_store_relaxed(field, prev & (~((uintptr_t)1 << bitidx)));

  if(mi_unlikely(prev == MI_BITMAP_FIELD_FULL)){
    uintptr_t next_idx = *super & (~((uintptr_t)1 << idx));
    *super = next_idx;
  }
  return true;
...
// single sized!!!
bool _mi_bitmap_try_find_from_claim_across_count_relaxed(mi_bitmap_t bitmap, const size_t bitmap_fields, mi_bitmap_super_t* super, const size_t count, mi_bitmap_index_t* bitmap_idx){
  UNUSED_RELEASE(bitmap_fields);
  ...
  mi_assert_internal(idx < bitmap_fields);
  _Atomic(uintptr_t)*field = &bitmap[idx];
  uintptr_t value = mi_atomic_load_relaxed(field);
  mi_assert_internal(value != MI_BITMAP_FIELD_FULL);
  size_t bitidx = mi_ctz(~value);  
  mi_assert_internal(bitidx != MI_INTPTR_BITS);

  size_t remain = MI_INTPTR_BITS - bitidx;

  if(mi_unlikely(remain < count)) {
    // next
    mi_assert_internal(idx + 1 < bitmap_fields);
    _Atomic(uintptr_t)*field_next = &bitmap[idx+1];
    uintptr_t next = mi_atomic_load_relaxed(field_next);
    mi_assert_internal(mi_ctz(~next) == 0);
    mi_assert_internal(mi_ctz(next) >= count - remain);

    mi_assert_internal((value & mi_bitmap_mask_(remain, bitidx)) == 0);
    mi_assert_internal((value | mi_bitmap_mask_(remain, bitidx)) == MI_BITMAP_FIELD_FULL);

    mi_assert_internal((next & mi_bitmap_mask_(count - remain, 0)) == 0);

    value = MI_BITMAP_FIELD_FULL;
    next |= mi_bitmap_mask_(count - remain, 0);

    mi_atomic_store_relaxed(field, value);
    mi_atomic_store_relaxed(field_next, next);
  } else {
    mi_assert_internal((value & mi_bitmap_mask_(count, bitidx)) == 0);
    value |= mi_bitmap_mask_(count, bitidx);
    mi_atomic_store_relaxed(field, value);
  }

  if (mi_unlikely(value == MI_BITMAP_FIELD_FULL)) {
    uintptr_t next_idx = *super | ((uintptr_t)1 << idx);
    *super = next_idx;
  }
  *bitmap_idx = mi_bitmap_index_create(idx, bitidx);
...
bool _mi_bitmap_unclaim_across_count_relaxed(mi_bitmap_t bitmap, const size_t bitmap_fields, mi_bitmap_index_t bitmap_idx, mi_bitmap_super_t* super, const size_t count) {
  UNUSED_RELEASE(bitmap_fields);
  ...
  if(mi_unlikely(remain < count)) {
    ...
    mi_assert_internal((next & mi_bitmap_mask_(count - remain, 0)) == mi_bitmap_mask_(count - remain, 0));
    mi_assert_internal((value & mi_bitmap_mask_(remain, bitidx)) == mi_bitmap_mask_(remain, bitidx));


    uintptr_t value_new = value & ~mi_bitmap_mask_(remain, bitidx);
    next &= ~mi_bitmap_mask_(count - remain, 0);

    mi_atomic_store_relaxed(field, value_new);
    mi_atomic_store_relaxed(field_next, next);
  } else {
    mi_assert_internal((value & mi_bitmap_mask_(count, bitidx)) == mi_bitmap_mask_(count, bitidx));
    uintptr_t value_new = value & ~mi_bitmap_mask_(count, bitidx);
    mi_atomic_store_relaxed(field, value_new);
  }
  *super &= (~((uintptr_t)1 << idx));
  return true;
}
