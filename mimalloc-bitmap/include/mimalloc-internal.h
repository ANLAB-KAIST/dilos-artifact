...
void*       _mi_bm_page_malloc(mi_heap_t* heap, mi_bm_page_t* page, size_t size) mi_attr_noexcept;  // called from `_mi_malloc_generic`
...
// are there immediately available blocks, i.e. blocks available on the free list.
static inline bool mi_bm_page_immediate_available(const mi_bm_page_t* page) {
  mi_assert_internal(check_bm_page(page));
  return (BM_TO_CONST_PAGE(page)->bm_super  != MI_BITMAP_FIELD_FULL);
}
static inline bool mi_page_immediate_available_generic(const mi_page_t* page) {
  return is_bm_page(page) ? mi_bm_page_immediate_available(PAGE_TO_CONST_BM(page)) : mi_page_immediate_available(page);
}
...
static inline size_t mi_popcnt(uintptr_t x) {
#if (INTPTR_MAX == LONG_MAX)
