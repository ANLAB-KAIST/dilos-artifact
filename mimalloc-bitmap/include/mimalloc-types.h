...
#define MI_BITMAP_FIELD_BITS   (MI_INTPTR_SIZE<<3)
#define MI_BITMAP_FIELD_FULL   (~((uintptr_t)0))   // all bits set


#define MI_BITMAP_FIELD_NUM(X)  (((X) + MI_BITMAP_FIELD_BITS - 1) >> (MI_INTPTR_SHIFT + 3))
#define MI_BITMAP_NUM_BITS(X) ((((X) - 1) >> 6) + 1)

// An atomic bitmap of `uintptr_t` fields
typedef uintptr_t           mi_bitmap_super_t;
...
typedef struct mi_page_s {
  ...
  union {
    ...
    mi_bitmap_t           bm_free;           // indirection of two bitmap: 1. free (first half), 2. local_free (second half)
  };
...
#define BM_CHECK_TYPES

#define PAGE_TO_BM(X) ((mi_bm_page_t*)(X))
#define PAGE_TO_CONST_BM(X) ((const mi_bm_page_t*)(X))
#define PAGE_TO_BM_DISPOSE(X) ((mi_bm_page_t*)(X));(X=NULL);

#ifdef BM_CHECK_TYPES
typedef struct mi_page_bm_s{
  mi_page_t inner;
} mi_bm_page_t;

#define BM_TO_PAGE(X) (&((X)->inner))
#define BM_TO_CONST_PAGE(X) (&((X)->inner))
#define BM_TO_PAGE_CONST(X) (&((X)->inner))
#else
typedef mi_page_t mi_bm_page_t;
#define BM_TO_PAGE(X) ((mi_page_t*)(X))
#define BM_TO_CONST_PAGE(X) ((const mi_page_t*)(X))
#define BM_TO_PAGE_CONST(X) ((mi_page_t* const)(X))
#endif


_Static_assert(sizeof(mi_bm_page_t) == sizeof(mi_page_t), "page size conflict");
