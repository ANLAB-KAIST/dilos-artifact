...
procfs_mount(mount* mp, const char *dev, int flags, const void* data)
{
    ...
    auto vm = make_shared<pseudo_dir_node>(inode_count++);
    vm->add("overcommit_memory", inode_count++, [] { return "1"; }); // OSv's overcommit_memory is similar to Linux's overcommit 1
    sys->add("vm", vm);
