...
static int rofs_read_without_cache(struct vnode *vnode, struct file *fp, struct uio *uio, int ioflag)
{
    ...
    void *buf = memory::alloc_phys_contiguous_aligned(BSIZE * block_count, 8, true);
    ...
    //     return error;
    // }
    char* buf_current = (char*) buf;
    while (block_count > 0){
        uint64_t block_count_this =  std::min<uint64_t>(block_count, 512);
        error = rofs_read_blocks(device, block, block_count_this, buf_current);
        if (error) {
            kprintf("[rofs_read] Error reading data\n");
            memory::free_phys_contiguous_aligned(buf);
            return error;
        }
        block += block_count_this;
        block_count -= block_count_this;
        buf_current += block_count_this * BSIZE;
    }
