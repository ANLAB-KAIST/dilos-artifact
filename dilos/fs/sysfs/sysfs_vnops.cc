...
#ifdef DDC
    auto infiniband = make_shared<pseudo_dir_node>(inode_count++);
    auto infiniband_verbs = make_shared<pseudo_dir_node>(inode_count++);

    ibsysfs_prepare(infiniband, infiniband_verbs, inode_count);
    auto class_ = make_shared<pseudo_dir_node>(inode_count++);
    class_->add("infiniband", infiniband);
    class_->add("infiniband_verbs", infiniband_verbs);
#endif
    auto* root = new pseudo_dir_node(vp->v_ino);
    root->add("devices", devices);
    root->add("osv", osv_extension);
#ifdef DDC
    root->add("class", class_);
#endif
