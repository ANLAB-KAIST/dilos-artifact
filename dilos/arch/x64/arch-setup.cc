...
void init_pat() {
    // Init MSR PAT
    uint64_t pat_msr = static_cast<uint64_t>(memory_type::WB) |          // PA0
                   static_cast<uint64_t>(memory_type::WC) << 8 |         // PA1
                   static_cast<uint64_t>(memory_type::UC_MINUS) << 16 |  // PA2
                   static_cast<uint64_t>(memory_type::UC) << 24 |        // PA3
                   static_cast<uint64_t>(memory_type::WB) << 32 |        // PA4
                   static_cast<uint64_t>(memory_type::WP) << 40 |        // PA5
                   static_cast<uint64_t>(memory_type::UC_MINUS) << 48 |  // PA6
                   static_cast<uint64_t>(memory_type::WT) << 56;         // PA7
    processor::wrmsr(msr::IA32_PAT, pat_msr);
}
}
...
#ifdef DDC
#include "core-ddc/virtio-uverbs.hh"
#endif
...
#ifdef DDC
    drvman->register_driver(virtio::uverbs::probe);
#endif
