...
void application::run_main()
{
    ...
    if (ddc::options::cpu_pin != -1){
        assert(ddc::options::cpu_pin < (int) sched::cpus.size());
        debug_early_u64("== Pinning CPU: ", (u64) ddc::options::cpu_pin);
        sched::thread::current()->pin(sched::cpus[ddc::options::cpu_pin]);
    }

    if(ddc::options::prefetcher_init_f != NULL){
       void (*func)(void) = (void (*)(void)) ddc::options::prefetcher_init_f;
       (*func)();
    }
