...
static void* malloc_large(size_t size, size_t alignment, bool block = true, bool contiguous = true)
{
    ...
    if (size >=  ddc::auto_remote_size && !contiguous){
        void* obj = ddc::mmap(nullptr, size, PROT_READ|PROT_WRITE,  MAP_PRIVATE | MAP_ANONYMOUS | MAP_DDC, -1, 0);
        return obj;
    }
...
reclaimer::reclaimer()
    : _oom_blocked(), _thread(sched::thread::make([&] { _do_reclaim(); }, sched::thread::attr().detached().name("reclaimer").stack(mmu::page_size*4)))
{
...
void reclaimer::_do_reclaim()
{
    ssize_t target;
    emergency_alloc_level = 1;
    _thread->pin(sched::cpus.back());

    while (true) {
        WITH_LOCK(free_page_ranges_lock) {
            if (!_oom_blocked.has_waiters())
        ...
        bool hard = _oom_blocked.has_waiters();

        _shrinker_loop(target, [&] { return hard; });
...
static size_t phys_list_next = 0;
void* get_phys_max(){
    mmu::phys max = 0;
    for(size_t i =0; i < phys_list_next; ++i){
        mmu::phys end = mmu::virt_to_phys(phys_list[i].first) + phys_list[i].second ;
        if(end > max)
            max = end;
    }
    return mmu::phys_to_virt(max);
}
...
{
#ifdef DDC
    if(ddc::alloc::pre_init(addr, size)){
        return;
    }
#endif
    ...
    assert(phys_list_next < max_phys_list);
    phys_list[phys_list_next].first = addr;
    phys_list[phys_list_next].second =  size;
    ++phys_list_next;
...
void free(void* object)
{
    DDC_HANDLER(object, mi_free, object);
...
{
    DDC_HANDLER(obj, mi_realloc, obj, size);
...
void reduce_total_memory(size_t memory){
    memory::total_memory.fetch_sub(memory);
    memory::watermark_lo = memory::stats::total() * 10 / 100;
}
}