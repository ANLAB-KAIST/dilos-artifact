...
uintptr_t find_hole(uintptr_t start, uintptr_t size, uintptr_t alignment = huge_page_size)
{
    bool small = size < alignment;
...
#ifdef DDC
            dev_vma* d_vma = dynamic_cast<dev_vma*>(&dead);
            if (d_vma){
                assert(d_vma->start() == start);
                assert(d_vma->end() == end);
                d_vma->do_munmap(start, end);
                ret += end - start;
            } else{
#endif
...
uintptr_t allocate(vma *v, uintptr_t start, size_t size, bool search, size_t alignment = huge_page_size)
{
    if (search) {
        ...
        start = find_hole(start, size, alignment);
    } else {
...
#ifdef DDC
    auto* dev_vma_ptr = dynamic_cast<dev_vma*>(vma);
    if(dev_vma_ptr != nullptr){
        start = dev_vma_ptr->start();
        WITH_LOCK(vma_list_mutex.for_write()) {
            vma_list.insert(*dev_vma_ptr);
        }
        return dev_vma_ptr->addr();
    }
#endif
...
void vm_fault(uintptr_t addr, exception_frame* ef)
{
    ...
    DDC_HANDLER(addr,ddc::vm_fault,addr,ef);
