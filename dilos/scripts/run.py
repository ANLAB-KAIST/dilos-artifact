...
def start_osv_qemu(options):
    ...
    for idx in range(int(options.nics)):
        ...
        if options.networking:
            if options.tap:
                ...
                args += ["-netdev", "tap,id=hn%d,script=%s,downscript=%s,vhost=on" % (idx, os.path.join(osv_base, "scripts/qemu-ifup.sh"),os.path.join(osv_base, "scripts/qemu-ifdown.sh"))]
            else:
    ...
    try:
        ...
        numa_cmd = []
        if options.numa_cpus and options.numa:
            numa_cmd += ["numactl", "-C", options.numa_cpus, "-m", options.numa]
        elif options.numa:
            numa_cmd += ["numactl", "-N", options.numa, "-m", options.numa]
        cmdline = numa_cmd + [qemu_path] + args
        if options.dry_run:
            print(format_args(cmdline))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--numa", action="store", help="numa node")
    parser.add_argument("--numa_cpus", action="store", help="numa cpus")
